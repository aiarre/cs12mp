...
describe("#model", () => {
  ...
  it("egg is initialized with valid parameters", () => {
    ...
    expect(model.egg?.x).toBeLessThanOrEqual(model.world.width - (model.egg?.width ?? 0));

    expect(model.egg?.y).toBeGreaterThanOrEqual(0);
    expect(model.egg?.y).toBeLessThanOrEqual(model.world.height - (model.egg?.height ?? 0));
    ...
    expect(model.egg?.speed).toBeGreaterThan(0);
  ...
  it("eggnemies are initialized with valid parameters", () => {
    ...
    expect(eggnemy.x).toBeLessThanOrEqual(model.world.width - (eggnemy.width ?? 0));

    expect(eggnemy.y).toBeGreaterThanOrEqual(0);
    expect(eggnemy.y).toBeLessThanOrEqual(model.world.height - (eggnemy.height ?? 0));
  ...
  it("settings are initialized with valid parameters", () => {
    ...
    expect(model.settings.defeatText).toBeDefined();
  ...
  it("checks if all parameters from settings.json are present in the model", () => {
    ...
    expect(model.egg?.speed).toStrictEqual(settings.egg.speed);
    ...
    expect(model.settings.bossSpawnThreshold).toStrictEqual(settings.game.bossSpawnThreshold);
    expect(model.settings.eggnemySpawningRatePerTick).toStrictEqual(settings.game.eggnemySpawningRatePerTick);
    expect(model.settings.victoryText).toStrictEqual(settings.game.victoryText);
    expect(model.settings.defeatText).toStrictEqual(settings.game.defeatText);
